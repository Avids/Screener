...
async function fromFinnhub(symbol: string, key: string) {
  ...
  if (j.s !== "ok" || !Array.isArray(j.t) || j.t.length === 0) throw new Error("finnhub empty");
...
async function fromPolygon(symbol: string, key: string) {
  ...
  if (!res.ok) throw new Error(`polygon failed ${res.status}`);
  const j = await res.json();
  if (!Array.isArray(j.results) || j.results.length === 0) throw new Error("polygon empty");
...
async function fetchHistory(symbol: string, finnhubKey?: string, polygonKey?: string) {
  ...
  if (polygonKey) {
    ...
    try {
      return { rows: await fromPolygon(symbol, polygonKey), provider: "Polygon", attempted };
    } catch {}
  }
  if (finnhubKey) {
    attempted.push("Finnhub");
    try {
      return { rows: await fromFinnhub(symbol, finnhubKey), provider: "Finnhub", attempted };
    } catch {}
  }

  attempted.push("Yahoo Finance");
  return { rows: await fromYahoo(symbol), provider: "Yahoo Finance", attempted };
}
...
export async function POST(req: NextRequest) {
  ...
  for (const batch of batches) {
    await Promise.all(batch.map(async (symbol) => {
      try {
        const fetched = await fetchHistory(symbol, body.finnhubKey?.trim(), body.polygonKey?.trim());
        providerUsed.add(fetched.provider);
  ...
  const provider = providerUsed.size ? Array.from(providerUsed).join(" + ") : "Unavailable";
